...
/**
 * The left-hand card: name, artwork, and the colour/capacity pickers that
 * decide which variant — and therefore which price — is in play.
 */
export function ProductGallery({ product, variant, onSelectColor, onSelectStorage }) {
  ...
  const finishLabel = product.colors.length === 1 ? "finish" : "finishes";
    ...
    <section className="min-w-0 rounded-2xl border border-hairline bg-surface p-5 sm:p-7">
      ...
      <div className="mt-6 text-center">
        <p className="text-sm text-ink-400">
          Available in {product.colors.length} {finishLabel}
        </p>
