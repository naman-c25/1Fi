...
function Row({ label, value, tone }) {
  return (
    <div className="flex items-center justify-between gap-4 py-1.5">
      ...
      <dd className={`numeric font-medium ${tone === "cash" ? "text-cash-600" : "text-ink-900"}`}>
        {value}
      </dd>
    </div>
  );
...
/**
 * Collapsible breakdown of the selected plan. Every figure is computed by the
 * Express API and arrives pre-formatted, so what the customer reads is exactly
 * what gets written onto the application.
 */
export function PlanSummary({ plan, variant }) {
  const [open, setOpen] = useState(false);
  const rounded = plan.finalInstalment.paise !== plan.monthlyAmount.paise;
  ...
  return (
    <div className="rounded-xl border border-hairline bg-canvas/60">
      ...
      >
        <span className="text-sm font-medium text-ink-700">Plan details &amp; backing fund</span>
        <svg
          viewBox="0 0 20 20"
          aria-hidden
      ...
      {open && (
        <div className="border-t border-hairline px-4 py-3">
          <dl className="text-sm">
            ...
            <Row
              label={`Monthly instalment × ${rounded ? plan.tenureMonths - 1 : plan.tenureMonths}`}
          ...
          {plan.fund && (
            <div className="mt-3 rounded-lg border border-brand-100 bg-brand-50/70 p-3">
              ...
              <p className="numeric mt-2 text-xs text-ink-500">
                Units worth{" "}
                <strong className="text-ink-700">{plan.pledgedAmount.display}</strong> stay pledged
                for {plan.tenureLabel} — they remain invested and keep earning.
              </p>
            </div>
          )}
        </div>
