...
export default function HomePage() {
  const { data: products, error, loading, reload } = useApi(() => getProducts(), []);

  useEffect(() => {
    document.title = "1Fi — Buy on EMI backed by your mutual funds";
  }, []);
  ...
  return (
    <div className="flex flex-col gap-10">
        ...
        <div className="grid gap-6 p-7 sm:p-10 lg:grid-cols-[1.35fr_1fr] lg:items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              >
                Browse {products ? `${products.length} phones` : "phones"}
              </Link>
              <a
                href={`${API_BASE}/api`}
                target="_blank"
      ...
      <section>
        <div className="mb-4">
          <h2 className="text-xl font-semibold tracking-tight text-ink-900">Available on EMI</h2>
          <p className="mt-0.5 text-sm text-ink-400">
            {loading
              ? "Loading the catalogue…"
              : `${products?.length ?? 0} products · every colour and capacity priced separately`}
          </p>
        </div>
