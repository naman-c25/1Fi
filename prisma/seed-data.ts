 ...
 *
 * Both `prisma/seed.ts` and `scripts/generate-images.ts` import this file, so
 * the image files on disk and the URLs stored in `variant_images` can never
 * drift apart.
 *
 * Prices here are written in plain rupees for readability; the seeder converts
 * them to paise before they touch the database.
 */

export interface SeedColor {
  name: string;
  /** Body colour of the rendered device. */
  hex: string;
  /** Screen wallpaper accent for the front view. */
  accentHex: string;
}
...
export interface SeedProduct {
  ...
  description: string;
  isNew: boolean;
  /** Shape of the rear camera housing in the generated artwork. */
  cameraStyle: "square-triple" | "pill-triple" | "bar-dual" | "circle-dual";
...
/**
 * The ladder every product offers: 0% up to 24 months, 10.5% beyond that,
 * with a flat cashback on each plan. Short tenures are backed by the liquid
 * fund, long tenures by the growth funds.
 */
function standardPlans(cashback: number): SeedPlan[] {
  return [
...
/** "256GB" -> "256gb" */
export function storageSlug(label: string): string {
